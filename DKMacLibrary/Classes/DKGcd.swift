...
public class DKGcd<T>
{
    ...
    public static func asyncOnMain(_  work:@escaping ()->())
    {
    ...
    /// work関数内の処理を必ずMainThread上でasync実行させるためのメソッド
    public static func async(qos:DispatchQoS.QoSClass = .default, _ work:@escaping ()->())
    {
    ...
    public static func  executeMultiWorksAsAsyncedOneWork(qos:DispatchQoS.QoSClass = .default, param:T?, works:[DKWork], completionHandler:@escaping (_ excuetedNumber:Int)->())
    {
    ...
    func async(completionHandler:@escaping (_ excuetedNumber:Int)->())
    {
        DispatchQueue.global(qos: qos).async {
